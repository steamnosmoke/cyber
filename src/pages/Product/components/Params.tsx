...
import { TCharacter, TTablet } from "../types";
...
import ProductGallery from "./Gallary";
import BlackLineButton from "buttons/components/BlackLineButton";

export default function ProductParams() {
  const { items: wishlist } = usePage<TProduct>("wishlist");
  const { items: cart } = usePage<TCartItem>("cart");
  ...
  const [wishLabel, setWishLabel] = useState("Add to Wishlist");
  ...
  const characters = useMemo<TCharacter[]>(
    () => [
      {
        label: "Screen size",
        img: screen,
  ...
  const setItem = useCallback(
    (color: string, memory: string) => {
      const currentVariant = product.variants.find(
        (item) => item.color === color && item.memory === memory
      );
      const variantId = String(product.variants.indexOf(currentVariant));
      ...
      const item: TCartItem = {
        ...
        discount: currentVariant
          ...
          : product.discount ?? 0,
        subTotal: currentVariant
          ? (currentVariant.price - (currentVariant.discount ?? 0)) * 1
          : (product.price - (product.discount ?? 0)) * 1,
        totalDiscount: currentVariant
          ? (currentVariant.discount ?? 0) * 1
  ...
  }, [setProduct, product, addToCart]);

  // const checkWishlist = useCallback(() => {
  //   const index: boolean = wishlist.some(
  //     (el) => el.objectId === product.objectId
  //   );
  //   if (index) {
  //     setWishLabel("Remove from Wishlist");
  //   } else {
  //     setWishLabel("Add to Wishlist");
  //   }
  // }, [wishlist, wishLabel, setWishLabel]);

  const onAddToWishlist = useCallback(() => {
    // checkWishlist();
    addToWishlist.mutate(product);
  }, [setProduct, product, addToWishlist]);
